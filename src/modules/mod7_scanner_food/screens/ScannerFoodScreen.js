...
export default function ScannerFoodScreen({ navigation }) {
  ...
  const { dailyTotals, getNextFood, resetFood } = useScannerDemo();
  const pct = Math.min(100, Math.round((dailyTotals.kcal / OBJETIVO) * 100));

  const scan = (demo) => {
    setLoading(true);
    setTimeout(() => {
      setLoading(false);
      const food = demo === 'reliable' ? getNextFood() : null;
      navigation.navigate('ScannerResultFood', { demo, food });
    }, 700);
  };
  ...
  return (
    <View style={s.container}>
      <View style={s.header}>
        <TouchableOpacity style={s.backBtn} onPress={() => { resetFood(); navigation.navigate('Home'); }}><Text style={s.backText}>←</Text></TouchableOpacity>
      ...
      <ScrollView contentContainerStyle={s.body} showsVerticalScrollIndicator={false}>
        <View style={s.dailyCard}>
          <View style={s.dailyTop}><Text style={s.dailyIcon}>🔥</Text><View style={{flex:1}}><Text style={s.dailyTitle}>Consumo diario aproximado</Text><Text style={s.dailySub}>{dailyTotals.kcal} / {OBJETIVO} kcal · {pct}% del objetivo</Text></View></View>
          <View style={s.progressTrack}><View style={[s.progressFill, { width: `${pct}%` }]} /></View>
          <View style={s.macroRow}><Macro label="Prot" value={`${dailyTotals.prot}g`}/><Macro label="Carbs" value={`${dailyTotals.carbs}g`}/><Macro label="Grasas" value={`${dailyTotals.grasas}g`}/></View>
        </View>
        ...
        <TouchableOpacity style={[s.option, s.ok]} disabled={loading} onPress={() => scan('reliable')}>
          <Text style={s.optionIcon}>✅</Text><View style={s.optionText}><Text style={s.optionTitle}>Resultado confiable</Text><Text style={s.optionSub}>Sale un alimento demo diferente y se puede agregar al día.</Text></View><Text style={s.go}>›</Text>
        </TouchableOpacity>
