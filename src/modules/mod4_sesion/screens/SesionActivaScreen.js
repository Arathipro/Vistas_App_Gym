...
function clonarEjercicios(lista) { return (lista || []).map((e, i) => ({ ...e, sesionUid: e.uid || `sesion-ej-${i}-${Date.now()}`, ordenSesion: i + 1, estadoSesion: e.estadoSesion || 'pendiente' })); }

function VideoBox({ ejercicio, visible }) {
  if (!visible) return null;
  const descripcion = ejercicio?.descripcion || 'Realiza el movimiento de forma controlada, mantén buena postura y evita usar impulso excesivo. Revisa posición inicial, recorrido y respiración antes de continuar.';
  return (
    <View style={s.videoBox}>
      <Text style={s.videoPlay}>▶</Text>
      <Text style={s.videoTitle}>Video y técnica</Text>
      <Text style={s.videoSub}>{ejercicio?.videoUrl ? 'Video cargado del catálogo' : 'Placeholder visual del video demostrativo'}</Text>
      <Text style={s.videoDesc}>{descripcion}</Text>
    </View>
  );
}

function GestionEjerciciosModal({ visible, ejercicios, ejIndex, registros, estadoActual, onClose, onAplicar, onCancelarEjercicio, onSustituirEjercicio, catalogo }) {
  ...
  function mover(i, dir) {
    ...
    if (local[i].estadoSesion === 'cancelado' || local[j].estadoSesion === 'cancelado') return;
    const arr = [...local];
    [arr[i], arr[j]] = [arr[j], arr[i]];
    setLocal(arr);
  }
  function puedeCancelar(i) {
    const e = local[i];
    if (e.estadoSesion === 'cancelado') return false;
    if (i !== ejIndex) return true;
    return estadoActual === 'preparacion' || estadoActual === 'serie';
  }
  function abrirCancelacion(i) {
    if (!puedeCancelar(i)) {
      Alert.alert('No disponible', 'El ejercicio actual solo puede cancelarse antes de terminar la ejecución de la serie.');
      return;
    }
    setSeleccion(i); setMotivo(MOTIVOS[0]); setUsarSustituto(false); setSustituto(null);
  }
  function confirmarCancelacion() {
    if (seleccion === null) return;
    const objetivo = local[seleccion];
    if (usarSustituto && sustituto) onSustituirEjercicio(seleccion, objetivo, { ...sustituto, sesionUid: `sub-${Date.now()}`, sustituyeA: objetivo.nombre, motivoSustitucion: motivo });
    else onCancelarEjercicio(seleccion, objetivo, motivo);
    setSeleccion(null);
  }
  ...
  return (
    <Modal transparent animationType="slide" visible={visible} onRequestClose={onClose}>
      <View style={s.modalOverlay}><View style={s.modalSheet}>
        ...
        <View style={s.modalHeader}><View><Text style={s.modalTitle}>Gestionar rutina activa</Text><Text style={s.modalSub}>Reordenar, cancelar o sustituir solo esta sesión</Text></View><TouchableOpacity style={s.closeBtn} onPress={onClose}><Text style={s.closeText}>✕</Text></TouchableOpacity></View>
        <ScrollView showsVerticalScrollIndicator={false}>
          {local.map((e, i) => {
            const regs = registros.filter(r => r.ejercicioKey === keyEj(e)).length;
            const actual = i === ejIndex;
            const cancelado = e.estadoSesion === 'cancelado';
            return <View key={keyEj(e) || i} style={[s.manageCard, actual && s.manageActual, cancelado && s.manageCanceled]}>
              <View style={{ flex: 1 }}><Text style={[s.manageName, cancelado && s.manageNameCanceled]}>{actual && !cancelado ? '● ' : ''}{e.icon || '🏋️'} {e.nombre}</Text><Text style={[s.manageSub, cancelado && s.manageSubCanceled]}>{cancelado ? `Cancelado · ${e.motivoCancelacion}` : `${actual ? 'En curso · ' : ''}${regs}/${totalSeries(e)} series posibles registradas`}</Text>{e.sustitutoNombre && <Text style={s.cancelInline}>Sustituido por {e.sustitutoNombre} · Motivo: {e.motivoCancelacion}</Text>}{e.sustituyeA && <Text style={s.cancelInline}>Sustituto de {e.sustituyeA} · {e.motivoSustitucion}</Text>}</View>
              {!cancelado && <View style={s.manageActions}><TouchableOpacity style={s.iconBtn} onPress={() => mover(i, -1)}><Text style={s.iconBtnText}>▲</Text></TouchableOpacity><TouchableOpacity style={s.iconBtn} onPress={() => mover(i, 1)}><Text style={s.iconBtnText}>▼</Text></TouchableOpacity><TouchableOpacity style={s.iconDanger} onPress={() => abrirCancelacion(i)}><Text style={s.iconDangerText}>⊘</Text></TouchableOpacity></View>}
            </View>;
          })}
          <TouchableOpacity style={s.btnSmallPrimaryFull} onPress={() => { onAplicar(local); onClose(); }}><Text style={s.btnSmallPrimaryText}>Aplicar cambios y cerrar</Text></TouchableOpacity>
        </ScrollView>
        {seleccion !== null && <View style={s.cancelPanel}><Text style={s.cancelTitle}>Cancelar: {local[seleccion]?.nombre}</Text><Text style={s.cancelSub}>Indica el motivo y decide si tendrá sustituto. Si no sustituyes, quedará marcado en rojo dentro del orden.</Text><View style={s.chips}>{MOTIVOS.map(m => <TouchableOpacity key={m} style={[s.chip, motivo === m && s.chipActive]} onPress={() => setMotivo(m)}><Text style={[s.chipText, motivo === m && s.chipTextActive]}>{m}</Text></TouchableOpacity>)}</View><TouchableOpacity style={s.checkRow} onPress={() => setUsarSustituto(v => !v)}><Text style={s.checkBox}>{usarSustituto ? '☑' : '☐'}</Text><Text style={s.checkText}>Sustituir por otro ejercicio</Text></TouchableOpacity>{usarSustituto && <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginVertical: 8 }}>{catalogo.slice(0, 10).map(e => <TouchableOpacity key={e.id} style={[s.subChip, sustituto?.id === e.id && s.subChipActive]} onPress={() => setSustituto(e)}><Text style={s.subChipText}>{e.icon || '🏋️'} {e.nombre}</Text></TouchableOpacity>)}</ScrollView>}<View style={s.formActions}><TouchableOpacity style={s.btnSmallSecondary} onPress={() => setSeleccion(null)}><Text style={s.btnSmallSecondaryText}>Volver</Text></TouchableOpacity><TouchableOpacity style={s.btnSmallPrimary} onPress={confirmarCancelacion}><Text style={s.btnSmallPrimaryText}>{usarSustituto ? 'Sustituir' : 'Cancelar sin sustituir'}</Text></TouchableOpacity></View></View>}
      </View></View>
    </Modal>
  );
}
...
export default function SesionActivaScreen({ navigation }) {
  ...
  const activos = ejerciciosSesion.filter(e => e.estadoSesion !== 'cancelado');
  const ejercicio = ejerciciosSesion[ejIndex];
  const total = totalSeries(ejercicio);
  const totalEsperado = activos.reduce((a, e) => a + totalSeries(e), 0);
  ...
  useEffect(() => { if (!ejercicio || ejercicio.estadoSesion === 'cancelado' || estado !== 'preparacion' || duracionFinal !== null) return; if (prep <= 0) { setEstado('serie'); setSerieSeg(0); return; } const id = setTimeout(() => setPrep(v => v - 1), 1000); return () => clearTimeout(id); }, [estado, prep, ejercicio, duracionFinal]);
  ...
  function avanzarAlSiguienteEjercicioDisponible(desdeIdx, lista = ejerciciosSesion) { const sig = lista.findIndex((e, i) => i >= desdeIdx && e.estadoSesion !== 'cancelado'); if (sig !== -1) { setEjIndex(sig); return; } finalizarCon(); }
  ...
  function guardarSolo() {
    ...
    if (!validarFormulario()) { pedirFormulario(); return null; }
  ...
  function cancelarEjercicio(idx, objetivo, motivo) { const regs = registros.filter(r => r.ejercicioKey === keyEj(objetivo)); const nuevoCancel = { id: `cancel-${Date.now()}`, nombre: objetivo.nombre, motivo, seriesRegistradas: regs.length, sustituido: false }; const nuevaLista = ejerciciosSesion.map((e, i) => i === idx ? { ...e, estadoSesion: 'cancelado', motivoCancelacion: motivo } : e); setCancelados(prev => [...prev, nuevoCancel]); setEjerciciosSesion(nuevaLista); if (idx === ejIndex) { setSerieActual(1); setPrep(PREP_DEFAULT); setSerieSeg(0); setDescansoSeg(0); setEstado('preparacion'); avanzarAlSiguienteEjercicioDisponible(idx + 1, nuevaLista); } }
  function sustituirEjercicio(idx, original, sustituto) { const cancelado = { ...original, estadoSesion: 'cancelado', motivoCancelacion: sustituto.motivoSustitucion, sustitutoNombre: sustituto.nombre }; const nuevo = { ...sustituto, series: original.series || sustituto.series || 3, repsMin: original.repsMin || sustituto.repsMin, repsMax: original.repsMax || sustituto.repsMax, descanso: original.descanso || sustituto.descanso, ordenSesion: idx + 1.5, estadoSesion: 'pendiente' }; const nuevaLista = [...ejerciciosSesion]; nuevaLista[idx] = cancelado; nuevaLista.splice(idx + 1, 0, nuevo); setCancelados(prev => [...prev, { id: `sub-${Date.now()}`, nombre: original.nombre, motivo: sustituto.motivoSustitucion, sustituido: true, sustituto: sustituto.nombre, seriesRegistradas: registros.filter(r => r.ejercicioKey === keyEj(original)).length }]); setEjerciciosSesion(nuevaLista); if (idx === ejIndex) { setEjIndex(idx + 1); setSerieActual(1); setPrep(PREP_DEFAULT); setSerieSeg(0); setDescansoSeg(0); setEstado('preparacion'); } }

  if (!activos.length && estado !== 'resumen') return <View style={s.container}><StatusBar barStyle="light-content" /><View style={s.center}><Text style={s.emptyIcon}>⏱️</Text><Text style={s.emptyTitle}>No hay sesión activa</Text><TouchableOpacity style={s.btnPrimary} onPress={() => navigation.navigate('Rutinas')}><Text style={s.btnPrimaryText}>Ir a rutinas</Text></TouchableOpacity></View></View>;

  if (estado === 'resumen') {
    const estimado = sesionActual?.tiempoEstimadoSeg || activos.reduce((a, e) => a + estimadoEj(e), 0);
    ...
    return <View style={s.container}><StatusBar barStyle="light-content" /><View style={s.header}><Text style={s.headerTitle}>Sesión finalizada</Text></View><ScrollView contentContainerStyle={s.body} showsVerticalScrollIndicator={false}>
      <View style={s.doneHero}><Text style={s.doneIcon}>✅</Text><Text style={s.doneTitle}>Entrenamiento guardado en memoria</Text><Text style={s.doneSub}>Datos simulados listos para alimentar el Módulo 8 con precargas.</Text></View>
      ...
      <Text style={s.sectionLabel}>Resumen por ejercicio</Text>{ejerciciosSesion.map((e, i) => { if (e.estadoSesion === 'cancelado') return <View key={keyEj(e) || i} style={s.cancelSummary}><Text style={s.cancelSummaryName}>⊘ {e.nombre}</Text><Text style={s.cancelSummarySub}>Motivo: {e.motivoCancelacion}{e.sustitutoNombre ? ` · Sustituido por ${e.sustitutoNombre}` : ' · Sin sustituto'}</Text></View>; const regs = registros.filter(r => r.ejercicioKey === keyEj(e)); const real = regs.reduce((a, r) => a + (Number(r.tiempoEjecucionSeg) || 0), 0) + Math.max(0, regs.length - 1) * descansoDe(e); const est = estimadoEj(e); const diff = real - est; return <View key={keyEj(e) || i} style={s.summaryExercise}><Text style={s.summaryName}>{e.icon || '🏋️'} {e.nombre}</Text>{e.sustituyeA && <Text style={s.substitutionText}>Sustituye a: {e.sustituyeA} · Motivo: {e.motivoSustitucion}</Text>}<Text style={s.summarySub}>{regs.length}/{totalSeries(e)} series registradas · real {fmt(real)} / estimado {fmt(est)}</Text><Text style={[s.timeDiff, diff <= 0 ? s.timeGood : s.timeBad]}>{diff <= 0 ? '🟢' : '🔴'} {diff <= 0 ? 'Antes' : 'Después'} por {fmt(Math.abs(diff))}</Text></View>; })}
  ...
  return <View style={s.container}><StatusBar barStyle="light-content" /><View style={s.header}><TouchableOpacity onPress={cancelarSesion} style={s.backBtn}><Text style={s.backText}>←</Text></TouchableOpacity><View style={s.headerCenter}><Text style={s.headerTitle}>{estadoLabel(estado)}</Text><Text style={s.headerSub}>{sesionActual?.rutinaNombre} · {sesionActual?.dia}</Text></View><Text style={s.timerMini}>{fmt(duracionMostrada)}</Text></View><ScrollView contentContainerStyle={s.body} showsVerticalScrollIndicator={false}>
    ...
    <TouchableOpacity style={s.videoTopButton} onPress={() => setMostrarVideo(v => !v)}><Text style={s.videoTopText}>{mostrarVideo ? 'Ocultar video y descripción' : 'Ver video y descripción del ejercicio 🎥'}</Text></TouchableOpacity><VideoBox ejercicio={ejercicio} visible={mostrarVideo} />
