...
function segundosEjercicio(e) {
  if ((e.categoria || '').toLowerCase() === 'cardio') return (Number(e.duracionMin) || 30) * 60;
  const s = Number(e.series) || 3;
  const descanso = Number(e.descanso) || 90;
  return (s * 60) + Math.max(0, s - 1) * descanso;
}
...
function estimarMinutos(rutina) {
  ...
  const segundos = dias.map(lista => lista.reduce((t, e) => t + segundosEjercicio(e), 0));
  return Math.round((segundos.reduce((a, b) => a + b, 0) / segundos.length) / 60);
}

export default function RutinasScreen({ navigation }) {
  const { rutinas, seleccionarRutina, abrirCatalogoLibre } = useRutinas();

  function irCatalogoLibre() {
    abrirCatalogoLibre();
  ...
  return (
    <View style={s.container}>
      ...
      <ScrollView contentContainerStyle={s.body} showsVerticalScrollIndicator={false}>
        ...
        <TouchableOpacity style={s.catalogCard} onPress={irCatalogoLibre}>
