...
import { getProfile, isProfileComplete, login } from '../services/authApiService';
import { useAppSession } from '../../../context/AppSessionContext';

export default function LoginScreen({ navigation, route }) {
  const { setUser, setPerfil, setToken } = useAppSession();
  ...
  async function handleLogin() {
    ...
    try {
      ...
      Alert.alert('Credenciales incorrectas', error.message || 'El correo o contraseña no son correctos. Verifica tus datos.', [{ text: 'Reintentar' }]);
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      <TextInput style={styles.input} placeholder="correo@email.com" placeholderTextColor="#666" value={email} onChangeText={setEmail} keyboardType="email-address" autoCapitalize="none" />

      <Text style={styles.label}>Contraseña</Text>
      <View style={styles.passContainer}>
        <TextInput style={[styles.input, { flex: 1, borderWidth: 0 }]} placeholder="Tu contraseña" placeholderTextColor="#666" value={pass} onChangeText={setPass} secureTextEntry={!showPass} />
      ...
      <TouchableOpacity style={[styles.btn, { opacity: email && pass && !loading ? 1 : 0.45 }]} onPress={handleLogin} disabled={!email || !pass || loading}>
        <Text style={styles.btnText}>{loading ? 'Entrando...' : 'Iniciar sesión'}</Text>
      </TouchableOpacity>

      <TouchableOpacity onPress={() => navigation.navigate('Register')}>
        <Text style={styles.link}>¿Sin cuenta? <Text style={styles.linkAccent}>Regístrate</Text></Text>
      </TouchableOpacity>
    </ScrollView>
  );
}
...
const styles = StyleSheet.create({
  ...
  passContainer: { backgroundColor: '#2a2a35', borderRadius: 10, borderWidth: 1, borderColor: '#333', flexDirection: 'row', alignItems: 'center', width: '100%' },
