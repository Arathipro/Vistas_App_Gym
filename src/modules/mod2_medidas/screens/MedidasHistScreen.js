...
export default function MedidasHistScreen({ navigation, route }) {
  ...
  // Editar → redirige al formulario completo con datos precargados
  function irAEditar(r) {
    navigation.navigate('MedidasReg', {
      user,
      modoEdicion: true,
      datos: { fecha: r.fecha, peso: r.peso, grasa: r.grasa, musculo: r.musculo, cintura: r.cintura },
    });
  }
  ...
  return (
    <View style={s.container}>
      ...
      <View style={s.header}>
        <TouchableOpacity onPress={() => navigation.navigate('Medidas', { user })} style={s.backBtn}>
          <Text style={s.backText}>←</Text>
        </TouchableOpacity>
        <Text style={s.headerTitle}>Historial de medidas</Text>
        <TouchableOpacity onPress={() => navigation.navigate('MedidasReg', { user })} style={s.actionBtn}>
          <Text style={s.actionText}>＋</Text>
        </TouchableOpacity>
      </View>

      <ScrollView contentContainerStyle={s.body} showsVerticalScrollIndicator={false}>
        <View style={s.badge}><Text style={s.badgeText}>RF09–RF11</Text></View>
            ...
            <View style={s.card}>
              <View style={s.cardHeader}>
                ...
                <View style={s.cardActions}>
                  <TouchableOpacity style={s.iconBtn} onPress={() => irAEditar(r)}>
                    <Text style={s.iconBtnText}>✎</Text>
                  </TouchableOpacity>
                  <TouchableOpacity style={s.iconBtnDanger} onPress={() => setConfirmElim(confirmElim === r.id ? null : r.id)}>
